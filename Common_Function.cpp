...
SDL_Surface*  ComFunc:: LoadImage(std::string file_path)
{
...
void ComFunc::ApplySurface(SDL_Surface* src, SDL_Surface* des, int x, int y)
{
	SDL_Rect offset;
	offset.x = x;
	offset.y = y;
...
void ComFunc::CleanUp()
{
	SDL_FreeSurface(g_screen);
	SDL_FreeSurface(g_bkground);
}
bool ComFunc::CheckCollision(const SDL_Rect& object1, const SDL_Rect& object2)
{
...
void ComFunc::ApplySurfaceClip(SDL_Surface* src, SDL_Surface* des, SDL_Rect* clip, int x, int y)
{
    SDL_Rect offset;
    offset.x = x;
    offset.y = y;
